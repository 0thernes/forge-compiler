...
describe("VM semantics", () => {
  it("short-circuits && (right side never evaluated)", () => {
    expect(
      run(
        `let hit = 0; fn f() { hit = 1; return 1; } let x = false && f(); print(hit);`,
      ),
    ).toEqual(["0"]);
  });
  ...
  it("short-circuits || (right side never evaluated)", () => {
    expect(
      run(
        `let hit = 0; fn f() { hit = 1; return 1; } let x = true || f(); print(hit);`,
      ),
    ).toEqual(["0"]);
  });

  it("normalizes logical results to 1/0", () => {
    expect(run(`print(5 && 7, " ", 0 || 3, " ", !0, " ", !9);`)).toEqual([
      "1 1 1 0",
    ]);
  });
  ...
  it("supports forward references between functions", () => {
    expect(
      run(
        `fn a(n) { return b(n) + 1; } fn b(n) { return n * 2; } print(a(5));`,
      ),
    ).toEqual(["11"]);
  });
  ...
  it("deep recursion works without host-stack overflow (iterative VM)", () => {
    expect(
      run(
        `fn down(n) { if (n == 0) { return 0; } return down(n - 1); } print(down(20000));`,
      ),
    ).toEqual(["0"]);
  });
