...
describe("lexer", () => {
  it("tokenizes a representative statement", () => {
    expect(types(`let x = 1 + 2.5;`)).toEqual([T.LET, T.IDENT, T.ASSIGN, T.NUM, T.PLUS, T.NUM, T.SEMI, T.EOF]);
  });

  it("distinguishes two-char operators from their prefixes", () => {
    expect(types(`a <= b == c != d >= e && f || !g += 1;`)).toEqual([
      T.IDENT, T.LTE, T.IDENT, T.EQ, T.IDENT, T.NEQ, T.IDENT, T.GTE, T.IDENT,
      T.AND, T.IDENT, T.OR, T.NOT, T.IDENT, T.PLUS_EQ, T.NUM, T.SEMI, T.EOF,
    ]);
  });
  ...
  it("skips // comments to end of line", () => {
    expect(types(`// nothing here\nlet x = 1;`)).toEqual([T.LET, T.IDENT, T.ASSIGN, T.NUM, T.SEMI, T.EOF]);
  });
